...
"""
Script for central training (train only a global model), could be replaced by fedavg by setting local_ep=1 and num_clients=1 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cfg", default="config/branch_cifar10.yaml", type=str)
    ...
    # logger
    log_dir = f"{config['metainfo']['work_dir']}/{config['metainfo']['exp_name']}_{config['dataset']['dirichlet']}_central"
    print(log_dir)
    ...
    if not config['test']:
        ...
        for round_i in tqdm(range(config["fl_opt"]["rounds"])):   
            ...
            # print( max(fed.waiting_time_record) - min(fed.waiting_time_record))
            fed.multi_scale_update()
            print_write([f"rounds: {round_i}, waiting_time: : {waiting_time} " ], log_file)
            ...
            # raise ValueError("Already Finished a epoch")
