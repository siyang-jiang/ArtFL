...
from fed_per import Fed_client, Fed_server



"""
Script for personalized training (train only a global model). 
"""
...
if __name__ == '__main__':
    ...
    data_root_dict = {
        ...
        'CIFAR10': '/mnt/ssd/xianghui/dataset/cifar_10'
        }
    ...
    parser.add_argument('--cfg', default="config/fedper.yaml", type=str)
    parser.add_argument("--exp_name", default="perFL", type=str, help="exp name")
    ...
    if dataset == "CUB": 
        per_client_data, per_client_label, test_data, test_label, cls_per_client = dataloader.CUB_FL(data_root, "base", config, aug=False)
        ...
        if config['dataset']['shot_few'] > 0:
            ...
            per_client_data, per_client_label, test_data, test_label, cls_per_client, num_per_cls_per_client, train_num_per_cls \
    ...
    for i in range(num_client):  
        p = Fed_client(
            network, criterion, config, 
            per_client_data, per_client_label, training_num_per_cls, 
            test_data, test_label, state_list, state_dict_list, eval_result_list, idx=i
            ) 
        p.daemon = True
        p.start()
    ...
    if not args.test:
        ...
        for round_i in range(config["fl_opt"]["rounds"]):
            ...
            # evaluate: meta-test
            fed.local_train(selected_idx, meta_test=True)
            all_results = np.array(copy.deepcopy(fed.eval_result_list)).transpose(1,0,2)   
            ...
            print(train_acc_per_cls, test_acc_per_cls)
