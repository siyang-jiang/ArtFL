...
class BBN_ResNet_Cifar(nn.Module):
    def __init__(self, block, num_blocks):
        ...
        self.layer1 = self._make_layer(block, 16, num_blocks[0], stride=1)      # output size (N*16*32*32)
        self.layer2 = self._make_layer(block, 32, num_blocks[1], stride=2)      # output size (N*32*16*16)
        self.layer3 = self._make_layer(block, 64, num_blocks[2] - 1, stride=2)  # output size (N*64*8*8)
        self.cb_block = block(self.in_planes, self.in_planes, stride=1)         # output size (N*64*8*8)
        self.rb_block = block(self.in_planes, self.in_planes, stride=1)         # output size (N*64*8*8)
