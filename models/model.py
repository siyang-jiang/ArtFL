...
class ResNet(nn.Module):
    """
    ResNet for image size 112
    """ 
    def __init__(self, block, layers, l2_norm, feat_dim=512, use_fc=False, dropout=False):
        super(ResNet, self).__init__()
    ...
    def forward(self, x, *args):
        ...
        x = self.layer2(x)
        x = self.layer3(x)
        x = self.layer4(x)  # (N, feat_dim, 6, 6)
...
class BBN_ResNet_Cifar(nn.Module):
    def __init__(self, block, num_blocks, l2_norm, feat_dim=128, input_channel=3):
        ...
        self.conv1 = nn.Conv2d(input_channel, channels[0], kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(channels[0])
...
class BasicBlock_another(nn.Module):
    expansion = 1
...
class BBN_ResNet_Cifar_another(nn.Module): 
