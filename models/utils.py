...
def init_models(config):
    ...
    """
    from models.model import BBN_ResNet_Cifar, BasicBlock, DotProduct_Classifier, ResNet, ResNet_224, Bottleneck
    ...
    if config["dataset"]["name"] == "CUB":
        if "ResNet10s" == config["networks"]["feat_model"]["def_file"]: 
            ...
            network["feat_model"] = ResNet_224(BasicBlock, [1,1,1,1], l2_norm=l2_norm, channels=[24,48,96,192]).to(device)  # 3/8 resnet10
            feat_dim = 192
        elif "ResNet10h" == config["networks"]["feat_model"]["def_file"]: 
            l2_norm = config["networks"]["feat_model"]["params"]["l2_norm"]
            network["feat_model"] = ResNet_224(BasicBlock, [1,1,1,1], l2_norm=l2_norm, channels=[32,64,128,256]).to(device) # half resnet10
            feat_dim = 256
        elif "ResNet10" == config["networks"]["feat_model"]["def_file"]: 
            l2_norm = config["networks"]["feat_model"]["params"]["l2_norm"]
            network["feat_model"] = ResNet_224(BasicBlock, [1,1,1,1], l2_norm=l2_norm).to(device)   # [64,128,256,512]
        elif "ResNet18" == config["networks"]["feat_model"]["def_file"]: 
            l2_norm = config["networks"]["feat_model"]["params"]["l2_norm"]
            network["feat_model"] = ResNet_224(BasicBlock, [2,2,2,2], l2_norm=l2_norm).to(device)
        elif "ResNet34" == config["networks"]["feat_model"]["def_file"]: 
            l2_norm = config["networks"]["feat_model"]["params"]["l2_norm"]
            network["feat_model"] = ResNet_224(BasicBlock, [3,4,6,3], l2_norm=l2_norm).to(device)
        else:
            raise NotImplementedError

    # for CIFAR
    if config["dataset"]["name"] in ["CIFAR10", "CIFAR100", "Speech", "imu", "statefarm", "depth"]:
        if config["dataset"]["name"] in ["Speech", "depth"] :
            input_channel = 1
        else:
            input_channel = 3
    ...
    if config["dataset"]["name"] in ["mini"]:
        # https://pytorch.org/hub/pytorch_vision_resnet/  
        if "ResNet10" in config["networks"]["feat_model"]["def_file"]: 
            l2_norm = config["networks"]["feat_model"]["params"]["l2_norm"]
            network["feat_model"] = ResNet(BasicBlock, [1,1,1,1], l2_norm, feat_dim).to(device) 
        elif "ResNet18" in config["networks"]["feat_model"]["def_file"]: 
            l2_norm = config["networks"]["feat_model"]["params"]["l2_norm"]
            network["feat_model"] = ResNet(BasicBlock, [2,2,2,2], l2_norm, feat_dim).to(device) 
        # elif "ResNet34" in config["networks"]["feat_model"]["def_file"]:
        #     l2_norm = config["networks"]["feat_model"]["params"]["l2_norm"]
        #     network["feat_model"] = ResNet_224(BasicBlock, [3,4,6,3], l2_norm=l2_norm).to(device)
        else:
            raise NotImplementedError

    if config["dataset"]["name"] in ["tiny"]:
        input_channel = 3
        if "ResNet32" in config["networks"]["feat_model"]["def_file"]: 
            # feat_dim = 512
            l2_norm = config["networks"]["feat_model"]["params"]["l2_norm"]
...
def init_optimizers(networks, config):
    ...
    for key, val in networks_defs.items():
        ...
        if optimizer_choice == 'adam':
            ...
            optimizer = torch.optim.Adamw({'lr': optim_params['lr']})
        else:
            raise RuntimeError
